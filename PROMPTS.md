...
**What it produced:**
- Recommended Problem 2 (Interview Agent) over:
  - Problem 1 (Redesign ABTalks) — judged mostly on UI/UX polish, not our team's strength
  - Problem 3 (Autonomous AI Creator) — requires genuine 48-hour unattended uptime, high infra risk on a hackathon timeline
- Full build plan proposed: deterministic question planner weighted by candidate mission attempts/skips, adaptive follow-up strategy, evidence-based feedback
...
**What it produced:** Full FastAPI backend built from scratch:
- `planner.py` — deterministic question-plan builder spanning curriculum modules/days, weighted by attempts/skips; verified against all 20 provided candidate profiles before any endpoint code was written
- `models.py` — Pydantic models matching `technical-spec.md`
- `llm.py` — LLM wrapper with mock fallback
- `prompts.py` — persona + prompt templates
- `main.py` — the `POST /api/interview` endpoint and session state machine
- `simulate.py` — end-to-end test harness
...
**What it produced:**
- `llm.py` rewritten to call Gemini instead of the original provider
- After reviewing the frontend's actual handoff spec, backend extended to:
  - Accept the frontend's lightweight `{role}` payload, synthesizing a full spanning candidate profile so the planner still guarantees 8+ questions/4+ days
  - Return `totalQuestions`/`questionNumber` so the frontend's counter isn't hardcoded
  - Add a deterministic score-cap rule (3+ non-answers caps readiness at 3/10), computed in code rather than trusted to the model
  - Force exactly 2 strengths/2 gaps with verbatim quoted snippets in the feedback prompt
  - Flagged and resolved a spec mismatch: frontend assumed 7 questions, real spec requires a minimum of 8
...
- Full Render deployment walkthrough:
  - Pushing the backend into a `/backend` subfolder of the shared repo
  - Creating the Render web service
  - Setting the root directory / build / start commands
  - Setting the API key as an environment variable
  - Verifying via `/health` and a live curl test
...
**What it produced:**
- Diagnosed as an uncaught exception
- Added a global exception handler
- Wrapped the LLM call in a try/except so any API failure falls back to a mock response instead of crashing the request
- Exposed the last real error via `GET /health` for fast debugging without digging through Render's log viewer
...
**What it produced:**
- Root cause found via the new `/health` error field:
  - Gemini's free tier was hitting its daily quota
  - Gemini's "thinking" models were separately consuming the output token budget on invisible internal reasoning, causing visible responses to truncate mid-sentence
...
**What it produced:**
- Backend migrated from Gemini to Groq (`llama-3.1-8b-instant`) — genuinely free-forever tier (14,400 requests/day vs. Gemini's 20/day), no thinking-token overhead
- Rewrote the mock fallback text to read as a normal, presentable interview response instead of visible "[MOCK]" debug text, so a worst-case API outage during judging would never look broken to a candidate
...
**What it produced:**
- Traced to the transition logic concatenating the model's same-topic follow-up text onto the next question whenever the follow-up cap forced an advance, even when the model's own strategy wasn't actually a "redirect"
- Fixed by only using that text as a transition when the strategy was genuinely "redirect"
...
**What it produced:**
- A 5-request test sequence run live against the deployed backend:
  1. Init
  2. Weak answer
  3. Strong answer
  4. Another strong answer
  5. Clarification request
- Confirmed clean single-question replies, correct question-count advancement, and correct clarification handling with no regressions
